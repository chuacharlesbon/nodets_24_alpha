...
export const logsViewController = async (req: Request, res: Response): Promise<void> => {
    try {
...
export const rateLimitLogsViewController = async (req: Request, res: Response): Promise<void> => {
    try {
        const logPath = path.join(process.cwd(), "shell", "rateLimitLogs.txt");
        const logs = await fs.readFile(logPath, "utf8");
